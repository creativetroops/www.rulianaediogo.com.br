...
class StepList extends Component {
  ...
  addChild = () => {
    ...
    if (this.state.childrenList.length >= rsvpConfig.maxChild) {
      this.setState({
        typedChild: '',
        errorChildList: `Você pode inserir no máximo ${rsvpConfig.maxChild} crianças.`,
      })
      return
    }
    this.setState({
  ...
  removeChild = (child) => {
    this.setState({
      typedChild: '',
      errorChildList: '',
      childrenList: this.state.childrenList.filter(childItem => childItem !== child),
    })
  }
  ...
  addPeople = () => {
    ...
    if (this.state.peopleList.length >= rsvpConfig.maxPeople) {
      this.setState({
        typedPeople: '',
        errorPeopleList: `Você pode inserir no máximo ${rsvpConfig.maxPeople} pessoas.`,
      })
      return
    }
    this.setState({
  ...
  removePeople = (people) => {
    if (this.state.peopleList.length === rsvpConfig.minPeople) {
      this.setState({
        errorPeopleList: `Você deve deixar pelo menos ${rsvpConfig.minPeople} pessoa na lista`,
      })
      return
    }
    this.setState({
      typedPeople: '',
      errorPeopleList: '',
      peopleList: this.state.peopleList.filter(peopleItem => peopleItem !== people),
    })
  }

  handleChange = (e) => {
    if (e.target.id === 'typedChild' || e.target.id === 'typedPeople') e.target.value = e.target.value.toUpperCase()
    this.setState({
  ...
  nextStep = () => {
    if (this.state.peopleList.length < rsvpConfig.minPeople) {
      this.props.changeError(
        `Você deve ter pelo menos ${rsvpConfig.minPeople} pessoa na lista de confirmação`,
      )
      return
    }
      ...
      <Fragment>
        ...
        <ul>
          {this.state.peopleList.map((people, index) => (
            <li key={index}>
              {people} <Button onClick={() => this.removePeople(people)}>Remover</Button>
            </li>
          ))}
        </ul>
        <Input id="typedPeople" onChange={this.handleChange} value={this.state.typedPeople} />
        ...
        <ul>
          {this.state.childrenList.map((child, index) => (
            <li key={index}>
              {child}{' '}
              <Button type="button" onClick={() => this.removeChild(child)}>
                Remover
              </Button>
            </li>
          ))}
        </ul>
        <Input id="typedChild" onChange={this.handleChange} value={this.state.typedChild} />
